...
from gui import match_game_visuals
...
class DemoClient(Frame):
        ...
        for index, game_style in enumerate(game_styles):
            if self.game_type == self.config["BATTLESHIPS_GAME_TYPE_ID"]:
                ...
                self.game_styles_listbox.insert(index, self.config["DOMINOES_GAME_STYLE_LISTBOX_TEXT"].format(game_style['GameStyleId'],
                                                                                                              ...
                                                                                                              game_style['GameTypeSpecificInfo']['Timeout ms']))
            elif self.game_type == self.config["MATCH_GAME_TYPE_ID"]:
                self.game_styles_listbox.insert(index, self.config["MATCH_GAME_STYLE_LISTBOX_TEXT"].format(game_style['GameStyleId'],
                                                                                                           game_style['Stake'],
                                                                                                           game_style['GameTypeSpecificInfo']['NumPairs'],
                                                                                                           game_style['GameTypeSpecificInfo']['Timeout ms']))
            else:
    ...
    def create_visuals(self):
        if self.game_type == self.config["NULL_GAME_TYPE_ID"]:
            ...
            self.player = match_game_visuals.MatchGameVisuals(self.middleFrameRight)  # Game Display Table
            self.opponent = match_game_visuals.MatchGameVisuals(self.middleFrameRight)  # Game Display Table
        else:
            raise ValueError('INVALID GAME TYPE PARAMETER')
