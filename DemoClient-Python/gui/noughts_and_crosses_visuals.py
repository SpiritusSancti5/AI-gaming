...
class NoughtsAndCrossesVisuals(tkinter.Canvas):
    ...
    def make_board(self, board):
        ...
        for i in range(9):
            if i == 3 or i == 6:
                output += "\n-----------"
            output += "| {0} |".format(board[i]) if (i - 1) % 3 == 0 else ("\n {0} ".format(board[i]) if i % 3 == 0 else " {0} ".format(board[i]))
        return output
